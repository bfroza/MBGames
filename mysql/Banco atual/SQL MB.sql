...
CREATE TABLE Jogos (
  ...
  imagem VARCHAR(255) NULL DEFAULT 'images.png',
  PRIMARY KEY (idJogos),
  ...
  CONSTRAINT fk_Jogos_Fornecedores1
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT chk_ValorMaiorQueZero CHECK (valor >= 1) -- Rule 
);
...
CREATE TABLE Cupons (
  ...
  CONSTRAINT chk_DescontoMaximo CHECK (desconto >= 0 AND desconto <= 50) -- Esse check é a rule, só deixa salvar se desconto estiver entre esses valores
);
